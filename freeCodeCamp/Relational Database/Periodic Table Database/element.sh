...
# This block handles the case where no argument is provided, prompting the user for input.
MAIN_PROGRAM() {
  if [[ -z $1 ]]
  then
...
# This block handles the main logic of the script, querying the database based on input.
PRINT_ELEMENT() {
  INPUT=$1
  if [[ ! $INPUT =~ ^[0-9]+$ ]]
  then
  ...
  # This block determines whether the input is a number (atomic number) or a string (element name) and constructs the appropriate SQL query.
  if [[ -z $ATOMIC_NUMBER ]]
  then
